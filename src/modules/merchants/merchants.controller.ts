...
import {
  MerchantLeaderboardQueryDto,
  MerchantLeaderboardMetric,
} from './dto/merchant-leaderboard-query.dto';
...
export class MerchantsController {
  ...
  @ApiOperation({
    ...
    description:
      'Allows an authenticated borrower or user to submit a merchant application for review.',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({ status: 400, description: 'User already active merchant or invalid payload.' })
  @ApiResponse({ status: 401, description: 'Unauthorized — valid JWT required.' })
  @ApiResponse({ status: 409, description: 'Application already pending under review.' })
  async apply(
    @CurrentUser() user: { wallet: string },
    @Body() dto: ApplyMerchantDto,
  ): Promise<{ success: boolean; data: MerchantApplicationResponseDto; message: string }> {
    const data = await this.merchantsService.apply(user.wallet, dto);
    return {
      success: true,
      data,
  ...
  async getLeaderboard(
    @Query() query: MerchantLeaderboardQueryDto,
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Active loans page size (default 20, max 100)',
  })
  @ApiQuery({
    name: 'offset',
    required: false,
    type: Number,
    description: 'Active loans records to skip (default 0)',
  })
  @ApiResponse({
    status: 200,
  ...
  @UseGuards(JwtAuthGuard)
  @ApiOperation({
    summary: 'Get a merchant time-series analytics (volume, default rate, avg loan size)',
  })
  @ApiParam({
    name: 'id',
    description: 'Merchant unique ID (UUID) or Stellar wallet address starting with G.',
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Number of trailing months to include (default 6, max 24)',
  })
  @ApiResponse({
    status: 200,
