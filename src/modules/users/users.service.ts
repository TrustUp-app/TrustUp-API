...
export class UsersService {
  ...
  /**
   * Self-service role upgrade to lp_provider.
   *
   * @param wallet - Stellar wallet address of the authenticated user
   */
  async becomeLp(wallet: string): Promise<{ wallet: string; role: UserRole; message: string }> {
    let user = await this.usersRepository.findByWallet(wallet);
    if (!user) {
      user = await this.usersRepository.create(wallet);
    }

    if (user.role === UserRole.LP_PROVIDER || user.role === UserRole.ADMIN) {
      return {
        wallet: user.wallet_address,
        role: user.role,
    ...
    const updated = await this.usersRepository.updateRole(user.id, UserRole.LP_PROVIDER);
    return {
