...
describe('IdempotencyInterceptor', () => {
  ...
  it('returns the cached response for the same key', async () => {
    ...
    await expect(lastValueFrom(interceptor.intercept(context(), next))).resolves.toEqual(cached);
    expect(next.handle).not.toHaveBeenCalled();
    expect(response.header).toHaveBeenCalledWith('X-Idempotent-Replayed', 'true');
  });
  ...
  it('does not cache handler errors', async () => {
    ...
    await expect(lastValueFrom(interceptor.intercept(context(), next))).rejects.toThrow('failed');
